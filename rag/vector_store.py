...
logger = setup_logger("aios.rag.vector_store")
...
class VectorStore(ABC):
    ...
    @abstractmethod
    def search(
        self, query_embedding: list[float], top_k: int = 5, filter: dict[str, Any] | None = None
    ) -> list[dict[str, Any]]:
        ...
...
class FAISSVectorStore(VectorStore):
    def __init__(self, dimension: int, index_path: str | Path | None = None) -> None:
        ...
        self._logger = setup_logger("aios.rag.vector_store.faiss")
        self._init_index()
        ...
        try:
            ...
            if self._index_path and self._index_path.exists():
                ...
                if not self._index.is_trained:
                    self._index.train(np.zeros((1, self._dim), dtype=np.float32))
                self._logger.info("Created new FAISS index (dim=%s)", self._dim)
        except ImportError:
            self._logger.warning("faiss not installed; using in-memory numpy fallback")
            self._index = None
    ...
    def add_batch(
        ...
        with self._lock:
            ...
            if not (arr.ndim == 2 and arr.shape[1] == self._dim):
                raise ValueError(f"Expected embeddings of shape (n, {self._dim}), got {arr.shape}")
...
class ChromaDBVectorStore(VectorStore):
    def __init__(self, collection_name: str, persist_path: str | Path | None = None) -> None:
        ...
        self._lock = threading.Lock()
        self._logger = setup_logger("aios.rag.vector_store.chromadb")
    ...
    def _init_collection(self) -> None:
        try:
            ...
            self._logger.warning("chromadb not installed; ChromaDB store unavailable")
    ...
    def add_batch(
        ...
        if self._collection is None:
            self._logger.warning("ChromaDB unavailable; skipping add_batch")
            return
        with self._lock:
